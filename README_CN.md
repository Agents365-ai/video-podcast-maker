...
┌─────────────────────────────────────────────────────────────────┐
│                    video-podcast-maker                          │
│         (完整工作流：主题 → 研究 → 脚本 → TTS → 视频)            │
│                                                                 │
│  ┌─────────────────────┐    ┌─────────────────────────────┐    │
│  │ remotion-design-    │    │   remotion-best-practices   │    │
│  │ master              │    │                             │    │
│  │ (UI 组件库)         │    │   (Remotion 框架知识)        │    │
│  │                     │    │                             │    │
│  │ • ChapterProgressBar│    │   • 动画 API                │    │
│  │ • FadeIn, SlideIn   │    │   • Composition 结构        │    │
│  │ • FullBleed         │    │   • 音频处理                │    │
│  │ • Title, ContentArea│    │   • 字幕渲染                │    │
│  │ • 主题色系统        │    │   • FFmpeg 集成             │    │
│  └─────────────────────┘    └─────────────────────────────┘    │
└─────────────────────────────────────────────────────────────────┘
```
...
| 技能 | 层级 | 用途 |
|------|------|------|
| **remotion-best-practices** | 基础层 | Remotion 框架通用知识（API、动画、音频、字幕等） |
| **remotion-design-master** | UI 层 | 可复用组件库（进度条、动画组件、布局、主题） |
| **video-podcast-maker** | 应用层 | 完整工作流，调用上面两个技能生成 B站视频播客 |

**依赖关系：** Step 8 创建视频组件时，本技能**必须使用** `remotion-design-master` 的组件。
